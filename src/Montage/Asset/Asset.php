...
abstract class Asset {
  ...
  public function handle(){
    ...
    foreach($src_path_list as $src_path){
      ...
      if($asset_map = $this->move($src_path,$dest_path)){
  ...
  protected function move(Path $src_path,Path $dest_path){
    ...
    foreach($src_path->createFileIterator() as $src_file){
      ...
      $public_path = new Path($dest_path,$this->relative_path,$relative_file->getPath(),$public_filename);
  ...
  protected function getType($extension){
    ...
    switch($ext){
      ...
      case 'css':
        ...
        $ret_type = self::TYPE_STYLESHEET;
        break;
        
      case 'javascript':
      
        $ret_type = self::TYPE_JAVASCRIPT;
        break;
    
      default:
    
        $ret_type = self::TYPE_IMAGE;
        break;
