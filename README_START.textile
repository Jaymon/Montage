...
<pre>
http://app.com/controller/method/$field
</pre>
...
<pre>
class foo_controller extends montage_controller {
...
Montage also cascades nicely. That means if it can't find a matching method, then it will call the controller's <code>handleIndex</code> method if it is defined. If it can't find a matching class, then it will use a default index controller's <code>handleMethod</code> method, if that doesn't exist, then it will call the index controller's <code>handleIndex</code> method.

There are two controller classes Montage will look for by default:

* <code>index_controller</code> - to handle default requests
* <code>error_controller</code> - to handle all error states

The <code>error_controller</code>'s methods correspond to the name of the exception they will be handling, (eg, <code>handleRuntimeException</code> will handle all raised RuntimeException exceptions), with <code>handleIndex</code> being the generic method for all exceptions that don't have a custom method to handle them.
...
the global class is named "app_start"

<pre>
class app_start extends montage_start {
...
The environment's start class name is the same value that the <code>MONTAGE_ENVIRONMENT</code> (eg, if your environment is foo, then foo_start is the name of the environment's start class) constant contains. Likewise, the controller's start class name is the same as the value that the <code>MONTAGE_CONTROLLER</code> constant contains (eg, if your controller is bar, then bar_start is the name of the controller's start class).
